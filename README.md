...
- `backup_keep`：远程旧包备份保留数量，默认 `5`。发布时旧包会备份为 `remote_filename.bak_时间戳`，清理时只删除同名包的旧备份；设置为 `0` 可关闭自动清理。
- `ssh_path` / `scp_path`：Windows OpenSSH 命令路径。

### 服务与健康检查配置

`service` 用于配置远程服务启停、状态检查和发布后健康检查：

- `stop_command`：停止服务命令。
- `start_command`：启动服务命令。
- `status_command`：状态检查命令。
- `startup_wait_seconds`：启动后等待秒数。
- `health_check_url`：发布后健康检查 URL，留空则跳过健康检查。该 URL 会在目标服务器上通过 `curl` 或 `wget` 访问，适合填写 `http://127.0.0.1:端口/...` 或服务器内网可访问地址。
- `health_check_retries`：健康检查重试次数，默认 `5`。
- `health_check_interval_seconds`：每次检查失败后的等待秒数，默认 `3`。
- `health_check_timeout_seconds`：单次 HTTP 检查超时秒数，默认 `5`。
...
页面顶部的 `发布检查` 页签用于在真正发布前检查目标服务器状态。检查项包括 SSH 连通性、远程目录是否存在、写入权限、磁盘空间、远程当前包以及服务状态命令执行结果。

发布前检查不会上传文件、不会备份、不会删除远程文件，也不会停止或启动服务。服务状态只执行当前配置里的 `status_command`；如果未配置状态检查命令，会显示为警告。

如果当前环境配置了多个副本，可以在检查页选择一个或多个副本。检查结果按目标服务器分组展示，状态分为通过、警告和失败。

### 备份回滚

页面顶部的 `备份回滚` 页签用于查看当前项目、环境、副本对应的远程备份包。点击 `刷新备份` 后，后端会在当前 `remote_dir` 中查询 `remote_filename.bak_*` 文件，并显示文件名、路径、时间和大小。

点击某个备份的 `回滚` 时，会先把当前远程包再次备份为 `remote_filename.bak_rollback_时间戳`，然后把选中的备份复制回当前包路径。回滚只接受当前发布目录下、且匹配当前 `remote_filename.bak_*` 的备份文件；不会复制任意服务器路径。

如果勾选 `回滚后重启服务`，回滚完成后会继续执行当前环境/副本的服务停机、启动和状态检查命令。未勾选时只恢复包文件，不自动重启。

### 发布记录

页面顶部的 `发布记录` 页签用于查看历史发布任务。每条记录会保存项目、环境、执行动作、副本、状态、开始时间、完成时间、耗时、失败原因和日志文件路径。点击记录后，可以在右侧查看对应的完整发布日志。

发布记录保存在本机 `data/release_history.json`，默认保留最近 500 条。日志正文仍保存在 `logs/` 目录，发布记录只保存日志文件路径，不会复制日志内容。

### 飞书通知

页面顶部的 `通知设置` 页签用于配置飞书机器人通知。当前简单版只支持飞书自定义机器人文本消息。

配置项：

- `enabled`：是否启用通知。
- `webhook_url`：飞书机器人的 Webhook 地址。
- `secret`：机器人开启签名校验时填写；未开启可留空。
- `notify_on_success`：发布成功时通知。
- `notify_on_failure`：发布失败时通知。
- `notify_on_cancelled`：发布被停止时通知。

通知只在发布任务结束后发送。通知发送失败不会改变发布任务结果，只会写入当前任务日志。可以先在页面点击 `发送测试通知` 验证机器人配置。
...
4. 执行状态检查命令。
5. 如果配置了 `health_check_url`，从目标服务器访问该 URL，HTTP 2xx 或 3xx 视为通过。
